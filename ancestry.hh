...
struct Ancestry {
    ...
    void fill_orders() {
        ...
        for (int i = 0; i < total_sample_size * number_of_loci; i++) {
            ...
            for (int j = 0; j < (int)break_points[loc].size() - 1; j++) {
                branches[i].order[j][branches[i].island] = 1;
            }
        }
        int child = 0;
        double mid = 0.0;
    ...
    void print_normalised_sfs_by_island() const {
        // =================================================
    ...
    void find_descendants(int branch_num, std::vector<int> &descendants) const {
      int child_num = 0;
      for (unsigned int i = 0; i < branches[branch_num].children.size(); i++) {
        child_num = branches[branch_num].children[i];
        if (branches[child_num].children.size() == 0) {
          if (std::find(descendants.begin(), descendants.end(), child_num) != descendants.end()){

          } else {
            descendants.push_back(child_num);
          }
        } else {
          find_descendants(child_num, descendants);
        }
      }
    }

    int get_index(std::vector<int> vec, int elt) const {
    ...
    void print_individual_mutations() const {

      int loc = 0, mutation_number = 0;
      double mid = 0.0, rate = 0.0;

      std::vector<int> tmp;
      std::vector<std::vector<int> >  mutations(number_of_loci*total_sample_size, tmp);

      std::vector<int> all_descendants;


      for (unsigned int i = 0; i < branches.size(); i++) {
        find_descendants(i, all_descendants);
      }

      std::vector<int> loci(all_descendants.size(), -1);

      for (unsigned int i = 0; i < loci.size(); i++) {
        loci[i] = branches[all_descendants[i]].locus;
      }
      ...
      for (unsigned int i = 0; i < branches.size(); i++) {
        ...
        mutation_number = gsl_ran_poisson(gen, rate);
        std::vector<int> descendants;

        find_descendants(i, descendants);

        if (branches[i].children.size() == 0) {
          descendants.push_back(i);
        }

        for (unsigned int j = 0; j < all_descendants.size(); j++) {
          if (std::find(descendants.begin(), descendants.end(), all_descendants[j]) != descendants.end()) {
            std::vector<int> add_mutations(mutation_number, 1);
            mutations[j].insert(mutations[j].end(), add_mutations.begin(), add_mutations.end());
          } else if (loci[j] == loc) {
            std::vector<int> add_mutations(mutation_number, 0);
            mutations[j].insert(mutations[j].end(), add_mutations.begin(), add_mutations.end());
          } else {
      ...
      for (unsigned int i = 0; i < mutations.size(); i++) {
        for (unsigned int j = 0; j < mutations[i].size(); j++) {
          std::cout << (int)mutations[i][j] << " ";
        }
        std::cout << loci[i];
        std::cout << std::endl;
      }
...
//==================================================================================
    ...
    void print_fixed_s(const int s) const {
        ...
        for (int i = 0; i < number_of_loci; i++) {
            for (int j = 0; j < s; j++) {
                double coin = gsl_rng_uniform(gen);
                double ub = bl[i][0] / total[i];
                int index = 0;
                while (coin > ub) {
                    index++;
                    ub += bl[i][index] / total[i];
                }
                sfs[i][index] += 1;
            }
            for (int j = 0; j < total_sample_size - 1; j++) {
